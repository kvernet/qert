...
void jacobi_rotate(std::vector<Complex> &A, uint32_t n, uint32_t p, uint32_t q)
{
...
double compute_entropy(const Complex *sv, uint32_t num_qubits, uint32_t k)
{
    ...
    {
        throw std::runtime_error("Entropy subsystem too large for Phase 1 implementation.");
    }
