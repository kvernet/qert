...
namespace qert
{
    ...
    Circuit::Circuit(uint32_t num_qubits, uint32_t depth)
        : num_qubits_(num_qubits), depth_(depth)
    {
    ...
    void Circuit::add_single_qubit_gate(
        GateType type, Qubit target, Depth depth, GateIndex gate_idx)
    {
        assert(depth < depth_);
    ...
    void Circuit::add_two_qubit_gate(
        GateType type, Qubit control, Qubit target,
        Depth depth, GateIndex gate_idx)
    {
        assert(depth < depth_);
    ...
    Circuit Circuit::brickwall_1d(uint32_t num_qubits, uint32_t depth)
    {
        ...
        for (uint32_t d = 0; d < depth; ++d)
        {
            ...
            if (d % 2 == 0)
            {
                ...
                {
                    circuit.add_two_qubit_gate(
                        GateType::RANDOM_SU4, q, q + 1, d, gate_idx++);
                }
            }
            else
            {
