...
namespace qert
{
    ...
    public:
        ...
        uint32_t num_gates() const { return static_cast<uint32_t>(gates_.size()); }

        // --- Gate access ---
        const GateDescriptor &gate(uint32_t index) const { return gates_[index]; }
        const std::vector<GateDescriptor> &gates() const { return gates_; }
        ...
        void add_single_qubit_gate(
            GateType type, Qubit target,
            Depth depth, GateIndex gate_idx);

        // Add a two-qubit gate.
        void add_two_qubit_gate(
