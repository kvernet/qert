                           ...
                           double (*estimator)(double const *, double const *))
{
    ...
    double prob_n = static_cast<double>(std::count_if(resample.begin(), resample.end(),
...
Approx::Approx(double value)
    : m_epsilon(static_cast<double>(std::numeric_limits<float>::epsilon()) * 100.), m_margin(0.0),
...
ScopedMessage::~ScopedMessage()
{
    if (!m_moved)
    {
        Detail::popScopedMessage(m_messageId);
    }
}
...
void GeneratorUntypedBase::skipToNthElementImpl(std::size_t n)
{
    for (size_t i = m_currentElementIndex; i < n; ++i)
    {
        ...
        if (!isValid)
        {
            Detail::throw_generator_exception("Coud not jump to Nth element: not enough elements");
        }
    }
}
...
Clara::Parser makeCommandLineParser(ConfigData &config)
{
    ...
    auto const setWarning = [&](std::string const &warning)
    {
        ...
        {
            config.warnings =
                static_cast<WarnAbout::What>(config.warnings | WarnAbout::InfiniteGenerator);
            return ParserResult::ok(ParseResultType::Matched);
        }
    ...
    auto const setBenchmarkSamples = [&](std::string const &samples)
    {
        ...
        if (*parsedSamples == 0)
        {
            return ParserResult::runtimeError("Benchmark samples must be greater than 0");
        }
    ...
    auto const setGeneratorFilter = [&](std::string const &generatorFilter)
    {
        if (generatorFilter != "*")
        {
            ...
            if (!parsedIndex)
            {
                return ParserResult::runtimeError("Could not parse '" + generatorFilter +
                                                  "' as generator index");
            }
        }
    ...
    auto const setPathFilter = [=, &config](std::string const &pathFilter)
    {
        ...
        {
            return ParserResult::runtimeError("Path filter '" + pathFilter + "' is too short");
        }
    ...
    auto cli =
        ...
        Opt(accept_many, setGeneratorFilter,
            "index spec")["-g"]["--generator-index"]("specify generator elements to try") |
...
{
static std::string
tryTranslators(std::vector<Detail::unique_ptr<IExceptionTranslator const>> const &translators)
{
  ...
  public:
    StreamRedirect()
        : m_cout(Catch::cout(), m_redirectedOut.get()),
          m_cerr(Catch::cerr(), m_redirectedErr.get()), m_clog(Catch::clog(), m_redirectedErr.get())
    {
    }
...
struct GeneratorTracker final : TestCaseTracking::TrackerBase, IGeneratorTracker
{
    ...
    GeneratorTracker(TestCaseTracking::NameAndLocation &&nameAndLocation, TrackerContext &ctx,
                     ITracker *parent, GeneratorBasePtr &&generator)
    ...
    void close() override
    {
        ...
        if (should_wait_for_child || (m_runState == CompletedSuccessfully &&
                                      !m_isFiltered // filtered generators cannot meaningfully move
                                                    // forward, as they would get past the filter
...
static CATCH_INTERNAL_THREAD_LOCAL SourceLineInfo g_lastKnownLineInfo("DummyLocation",
...
void RunContext::assertionEnded(AssertionResult &&result)
{
    ...
    else if (!result.succeeded())
    {
        ...
        { // Read from a shared state established before the threads could start, this is fine
                                                      ...
                                                      Generators::GeneratorBasePtr &&generator)
{
    ...
    if (m_config->warnAboutInfiniteGenerators() && !generator->isFinite())
    {
        ...
        INTERNAL_CATCH_MSG("FAIL", Catch::ResultWas::ExplicitFailure,
                           Catch::ResultDisposition::Normal, "GENERATE() would run infinitely");
    }
    ...
    auto newTracker = Catch::Detail::make_unique<Generators::GeneratorTracker>(
        CATCH_MOVE(nameAndLoc), m_trackerContext, &currentTracker, CATCH_MOVE(generator));
...
void RunContext::runCurrentTest()
{
    ...
    CATCH_CATCH_ALL
    {
        ...
        if (m_shouldReportUnexpected)
        {
            ...
            handleUnexpectedInflightException(makeDummyAssertionInfo(), translateActiveException(),
                                              dummyReaction);
        }
    }
    updateTotalsFromAtomics();
...
AssertionInfo RunContext::makeDummyAssertionInfo()
{
    ...
    return AssertionInfo{
        testCaseJustStarted ? "TEST_CASE"_sr : StringRef(), Detail::g_lastKnownLineInfo,
...
Column::const_iterator::const_iterator(Column const &column)
    : m_column(column), m_lineStart(column.m_string.begin()), m_lineEnd(column.m_string.begin()),
  ...
  public:
    ...
    ConsoleAssertionPrinter(std::ostream &_stream, AssertionStats const &_stats,
        ...
        : stream(_stream), stats(_stats), result(_stats.assertionResult), colour(Colour::None),
          messages(_stats.infoMessages), colourImpl(colourImpl_),
